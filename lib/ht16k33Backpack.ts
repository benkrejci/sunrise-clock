...
export class Backpack extends EventEmitter {
    ...
    public constructor(bus: number, private address: number) {
        ...
        this.wire = open(bus, (err) => {
            if (err == null) {
                ...
                this.executeCommand(
                    HT16K33_CMD_OSCILATOR | HT16K33_CMD_OSCILATOR_ON,
                    'HT16K33_CMD_OSCILATOR_ON',
                )
                    // Turn blink off
    ...
    public setBlinkrate(rate: Blinkrate): Promise<void> {
        ...
        return this.executeCommand(
            HT16K33_CMD_DISPLAY | this.state | (rate << 1),
            'HT16K33_CMD_DISPLAY',
        )
    }
    ...
    public setBrightness(brightness: number): Promise<void> {
        ...
        return this.executeCommand(
            HT16K33_CMD_BRIGHTNESS | brightness,
    ...
    public setBufferBlock(block: number, value: number): void {
        ...
        if (block < 0 || block >= UINT16_BUFFER_SIZE) {
            ...
            throw new Error(
                `Buffer over- or underflow, tried to write block ${block}, which is out of range of 0-${UINT16_BUFFER_SIZE}.`,
            )
        }
    ...
    private executeCommand(
        ...
        return new Promise<void>((resolve, reject) => {
            this.wire.writeByte(this.address, cmd, arg, (err) => {
                if (err != null) {
                    debug(
                        `[err] unable to execute command "${debugName}"!`,
                        err,
                    )
                    reject(err)
                }
